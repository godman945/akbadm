...
public class PfdYearBonus extends APfdBonusItem{

	private final EnumPfdBonusItem pfdBonusItem = EnumPfdBonusItem.EVERY_YEAR_BONUS;
...
//		float payBonus = this.getPayBonus(pfdId, pfdPayType, year, quarter);
...
//				bonusRecord.setDeductBonus(payBonus);   //***** 目前沒用到這欄，應該以後也不會用到
	...
	private float getPayBonus(String pfdId, String pfdPayType, int year, int quarter) {
		...
		// 取已領取季獎金
		float payQuarterBonus = pfdBonusRecordService.findPayBonus(pfdId, pfdPayType, year, quarter, EnumPfdBonusItem.EVERY_QUARTER_BONUS.getItemType());
				
		return (payMonthBonus+payQuarterBonus);
	}

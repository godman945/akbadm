 ...
 * 餘額低於 300 元時，發出餘額偏低
 */
public class NoticeRemainTooLow extends ANotice {

	private int LIMIT_OF_REMAIN = 300; //餘額條件設定

	private EnumCategory enumCategory = EnumCategory.REMAIN_TOO_LOW;
        ...
        for (PfpCustomerInfo pfpCustomerInfo: targetList) {
            ...
            try {
    			...
    			if (PfdUserAdAccountRefList.size()>0) {
    				...
    				conditionsMap.put("content","%帳戶餘額過低%");
    				...
    				if(pfdBoardList.isEmpty()){
    					...
    					//PFD 公告
    					String content = "廣告帳戶 <span style=\"color:#1d5ed6;\">" + pfpCustomerInfo.getCustomerInfoTitle() + "</span> ，帳戶餘額過低，";
    					...
    					board2.setBoardContent("廣告帳戶 <span style=\"color:#1d5ed6;\">" + pfpCustomerInfo.getCustomerInfoTitle() + "</span> ，帳戶餘額過低，請立即進行加值以避免影響您的廣告播放!");

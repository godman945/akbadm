...
import com.pchome.akbadm.db.service.pfbx.report.IPfbxStyleReportService;
...
public class PfbStyleReportAction extends BaseAction {

	private static final long serialVersionUID = 1L;

	private IPfbxCustomerInfoService pfbxCustomerInfoService;
	private IPfbxStyleReportService pfbxStyleReportService;
    ...
    public String execute() throws Exception {
    	...
    	if (StringUtils.isEmpty(startDate) && StringUtils.isEmpty(endDate)) {
	    	...
	    	voList = pfbxStyleReportService.getPfbxStyleReportByCondition(conditionMap);
    ...
    private void makeDownloadReportData() throws Exception {
    	...
    	String filename="廣告樣式成效報表_" + dformat.format(new Date()) + ".csv";
    	DecimalFormat df = new DecimalFormat("0.00");
    	DecimalFormat df2 = new DecimalFormat("###,###,###,###");
    	DecimalFormat df3 = new DecimalFormat("###,###,###,##0.00");
    	String[] tableHeadArray = {"期間","帳戶編號","帳戶類別","網站名稱","廣告樣式","廣告曝光數","廣告點擊數","廣告點擊率","單次點擊收益","千次曝光收益","預估收益"};
    	
    	StringBuffer content=new StringBuffer();
    	
    	content.append("報表名稱:,廣告樣式成效");
		content.append("\n\n");
		content.append("廣告樣式:,");
		if (StringUtils.isNotBlank(style)){
			content.append(style + "廣告");
		} else {
			content.append("全部");
		}
		content.append("\n\n");
		...
		for (int i=0; i<voList.size(); i++) {
			...
			content.append("\"" + vo.getAdPvclkStyle() + "\",");
	...
	public IPfbxStyleReportService getPfbxStyleReportService() {
		return pfbxStyleReportService;
	}

	public void setPfbxStyleReportService(
			IPfbxStyleReportService pfbxStyleReportService) {
		this.pfbxStyleReportService = pfbxStyleReportService;
	}

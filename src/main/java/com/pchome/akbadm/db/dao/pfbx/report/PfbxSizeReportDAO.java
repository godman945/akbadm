...
public class PfbxSizeReportDAO extends BaseDAO<PfbxReportVO, String> implements IPfbxSizeReportDAO {
    ...
    public List<PfbxReportVO> getPfbxSizeReportByCondition(final Map<String, String> conditionMap) {
		...
		//			hql.append("  sum(adInvalidClkPrice), ");
					...
					hql.append(" /(select sum(d.ad_clk_price) - sum(d.ad_invalid_clk_price) from pfbx_ad_size_report d ");
		...
		//			hql.append(" adPvclkCustomer, ");
					...
					if (StringUtils.isNotEmpty(conditionMap.get("startDate"))) {
						hql.append(" and ad_pvclk_date between '"+conditionMap.get("startDate")+"'");
					}
					...
					if (StringUtils.isNotEmpty(conditionMap.get("size"))) {
						hql.append(" and a.ad_pvclk_size= '"+conditionMap.get("size")+"'");
					}
					...
					hql.append(" group by a.ad_pvclk_date, a.customer_info_id, a.ad_pvclk_size ");
					hql.append(" order by a.ad_pvclk_date, a.customer_info_id, a.ad_pvclk_size ");
					...
					for (int i=0; i<dataList.size(); i++) {
						...
						vo.setAdPvclkSize((objArray[5]).toString());

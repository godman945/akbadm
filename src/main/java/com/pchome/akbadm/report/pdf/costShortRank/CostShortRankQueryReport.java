package com.pchome.akbadm.report.pdf.costShortRank;
...
public class CostShortRankQueryReport extends ACostShortRankQueryTemplate{
	...
	public void buildPdf(Document doc, List<String> queryCondition) throws Exception {
		...
		// 表格欄位名稱
		String[] colName = {"排名", "帳戶名稱", "廣告活動", "每日花費上線", "曝光數", "點選率", "點選次數", "未達費用", "費用"};
		...
		List<AdPvclkVO> adPvclkVOs = pfpAdPvclkService.findAdPvclkCostShortRank(startDate, endDate, Integer.parseInt(pageSize));
		...
		if(adPvclkVOs != null && adPvclkVOs.size() > 0){
			...
			for(AdPvclkVO vo:adPvclkVOs){
				...
				if(showTitle){
					...
					// 標題
					this.addReportTitle(doc, "未達每日花費上限報表", tableWidth, String.valueOf(currentPageNum));
					// 副標題
	...
	private PdfPTable buildReportContentTable(AdPvclkVO vo, int[] colSize, int tableWidth, int rank) throws Exception{
		...
		// 未達每日花費上限報表
		cell = this.buildReportContentCell(String.valueOf((int)vo.getShortCost()), this.getFont8());
		table.addCell(cell);

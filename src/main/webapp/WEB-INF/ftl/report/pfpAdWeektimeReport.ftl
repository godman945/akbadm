...
<h2><img src="<@s.url value="/" />html/img/iconcr.gif" hspace="2" vspace="12" align="absmiddle" />廣告播放時段成效查詢</h2>

<form action="pfpAdWeektimeSearch.html" method="post">
    ...
    <tr>
        <td>查詢日期 : 
            ...
            <select id="searchTime" name="searchTime">
                ...
                <option value="T">時段</option>
		    </select>
		    <input type="hidden" id="time" value="${searchTime!}" />
		    &nbsp;&nbsp;
...
<table id="tableView" class="table01 tablesorter">
    ...
    <#list totalList as totalVO>	
	    <tr>
	    	<td class="td09" ></td>
	        <#if searchTime == 'T'>
	        	<td class="td09" >${totalVO.time}</td>
	        <#else>
	        	<td class="td09" >${totalVO.week}</td>
	        </#if>

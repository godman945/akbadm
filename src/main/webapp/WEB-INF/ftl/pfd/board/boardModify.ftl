...
<br>
<form action="doPfdBoardUpdate.html" method="post">
...
<table width="500" class="table01">
        ...
        <td class="td02">
            <select id="pfdCustomerInfoId" name="pfdCustomerInfoId">
                ...
                <#list pfdCustomerInfoMap?keys as key>
                    <option value="${key}" <#if pfdCustomerInfoId?exists && pfdCustomerInfoId = key>selected</#if>>${pfdCustomerInfoMap[key]}</option>
                </#list>
            </select>
        </td>
    </tr>
    <tr>
        ...
        <td class="td02">
            <select id="pfdUserId" name="pfdUserId">
            <#list pfdUserMap?keys as key>
                    <option value="${key}" <#if pfdUserId?exists && pfdUserId = key>selected</#if>>${pfdUserMap[key]}</option>
            </#list>
            </select>
    </tr>
    <tr>

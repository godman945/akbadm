...
<h2><img src="<@s.url value="/" />html/img/iconcr.gif" hspace="2" vspace="12" align="absmiddle" />尺寸成效查詢</h2>
    ...
    <tr>
        <td>查詢日期 : 
            ...
            <option value= "">---請選擇---</option>
		    	<#list sizeMap?keys as skey>
		    		<option value="${skey}" <#if size?exists && size = skey>selected="selected" </#if>>${sizeMap[skey]}</option>
		    	</#list>
		    </select>
		    <input type="hidden" id="searchSize" value="${size!}" />
		    <br/>
            <BR>查詢條件 :
    ...
    <tbody>
	    <#list voList as data>
	    	<tr>
		        ...
		        <td class="td09"><a onclick="openClick('${data.customerInfoId}','${data.adPvclkDate}','${data.adPvclkSize}');">${data.adPvclkSize}</td>
